...
class RequirementExtractor:
    ...
    def _determine_priority(self, line: str) -> TestCasePriority:
        ...
        if any(
            keyword in line_lower for keyword in ["critical", "essential", "mandatory"]
        ):
            return TestCasePriority.CRITICAL
        elif any(
            keyword in line_lower for keyword in ["important", "high", "priority"]
        ):
            return TestCasePriority.HIGH
        elif any(
            keyword in line_lower for keyword in ["low", "optional", "nice to have"]
        ):
    ...
    def _identify_compliance_standards(self, line: str) -> List[str]:
        ...
        if any(
            keyword in line_lower for keyword in ["hipaa", "privacy", "patient data"]
        ):
            standards.append("hipaa")
        if any(
            keyword in line_lower for keyword in ["fda", "medical device", "regulation"]
        ):
