...
class QualityMetrics(BaseModel):
    ...
    completeness_score: float = Field(
        ge=0.0, le=1.0, description="Test case completeness score"
    )
    accuracy_score: float = Field(
        ge=0.0, le=1.0, description="Test case accuracy score"
    )
    traceability_score: float = Field(
        ge=0.0, le=1.0, description="Requirement traceability score"
    )
    compliance_score: float = Field(
        ge=0.0, le=1.0, description="Compliance coverage score"
    )
    ...
    average_test_cases_per_requirement: float = Field(
        ge=0.0, description="Average test cases per requirement"
    )
...
class ToolchainIntegration(BaseModel):
    ...
    platform: str = Field(
        ..., description="Integration platform (Jira, Polarion, etc.)"
    )
