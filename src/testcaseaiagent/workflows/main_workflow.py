...
class HealthcareTestCaseGenerator:
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {"continue": "compliance_mapper", "error": "error_handler"},
        )
        workflow.add_conditional_edges(
            "compliance_mapper",
            self._should_continue_after_compliance,
            {"continue": "test_generator", "error": "error_handler"},
        )
        workflow.add_conditional_edges(
            "test_generator",
            self._should_continue_after_tests,
            {"continue": "quality_validator", "error": "error_handler"},
        )
    ...
    def _compliance_mapper_node(self, state: GraphState) -> GraphState:
        ...
        try:
            mappings = self.compliance_mapper.map_requirements_to_compliance(
                state.extracted_requirements, state.compliance_standards
            )
    ...
    def _test_generator_node(self, state: GraphState) -> GraphState:
        ...
        try:
            test_cases = self.test_generator.generate_test_cases(
                state.extracted_requirements, state.compliance_mappings
            )
    ...
    def _error_handler_node(self, state: GraphState) -> GraphState:
        ...
        state.final_report = {
            ...
            "completed_steps": [
                ...
                if step.status == ProcessingStatus.COMPLETED
            ],
        }
        return state
    ...
    def _should_continue_after_compliance(self, state: GraphState) -> str:
        ...
        if not state.compliance_mappings:
            logger.warning(
                "No compliance mappings generated, continuing with test generation"
            )
        return "continue"
        ...
        return {
            ...
            "processing_summary": {
                ...
                "quality_metrics": (
                    {
                        "completeness_score": (
                            state.quality_metrics.completeness_score
                            if state.quality_metrics
                            else 0.0
                        ),
                        "accuracy_score": (
                            state.quality_metrics.accuracy_score
                            if state.quality_metrics
                            else 0.0
                        ),
                        "traceability_score": (
                            state.quality_metrics.traceability_score
                            if state.quality_metrics
                            else 0.0
                        ),
                        "compliance_score": (
                            state.quality_metrics.compliance_score
                            if state.quality_metrics
                            else 0.0
                        ),
                        "coverage_percentage": (
                            state.quality_metrics.coverage_percentage
                            if state.quality_metrics
                            else 0.0
                        ),
                    }
    ...
    def process_documents(
        ...
        try:
            ...
            initial_state = GraphState(
                ...
                or settings.default_compliance_standards,
                processing_config={
            ...
            return {
                ...
                "session_id": session_id,
                "requirements": [
                    req.model_dump() for req in final_state.extracted_requirements
                ],
                "test_cases": [
                    tc.model_dump() for tc in final_state.generated_test_cases
                ],
                "quality_metrics": (
                    final_state.quality_metrics.model_dump()
